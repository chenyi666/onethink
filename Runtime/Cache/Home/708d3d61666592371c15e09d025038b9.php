    ...
    <div class="container-fluid">
            <div id="box">
                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="row noticeList">
                        ...
                        <input type="hidden" class="doucment_id" value="<?php echo ($vo["id"]); ?>">
                        <input type="submit" class="btn btn-success enlist" value="点击报名">
...
<script type="application/javascript">
        $(function(){
            ...
            $(".get_more").click(function(){
                $.post("<?php echo U('index/active');?>",{p:p+1},function(data){
                    if(data.status == 1){
                        p++;
                        var html = '';
            ...
            $(".enlist").click(function () {
                //获取文章id
                var id=$(this).closest(".row").find(".doucment_id").val();
               /* console.debug(id);*/
                //发送ajax请求
                $.post("<?php echo U('index/enlist');?>",{id:id},function (data) {
                    if(data.status==1){
                        alert('报名成功')
                    }else{
                        alert('请不要重复报名')
